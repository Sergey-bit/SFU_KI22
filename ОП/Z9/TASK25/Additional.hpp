...
namespace dop
{
	...
	template<typename T = float>
	void print_1DArray(const T* array, const size_t length)
	{
		for (const T* i = array; i < array + length; i++)
		{
	...
	template<typename T = float>
	void print_2DArray(const T** array, const size_t length, const size_t sublen)
	{
		for (const T** i = array; i < array + length; i++)
		{
			for (const T* j = *i; j < i + sublen; j++)
			{
				std::cout << "Array[" << i - array << "][" << j - *i << "]=" << *j << std::endl;
			}
		}
	}
